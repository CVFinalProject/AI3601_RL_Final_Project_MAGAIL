...
from multiagent_benchmark_53.eval_sampler import ParallelPathSampler
from multiagent_benchmark_53.fixed_traffic_simulator import FixedMATrafficSim
from multiagent_benchmark_53.math_utils import DiscreteFrechet
import pickle as pk

sys.setrecursionlimit(25000)

def benchmark(vehicle_id_groups_list, agent_num, ngsim_path, psgail):
...
if __name__ == '__main__':
    filename = './best_agent/psgail_1772_gail_2_264_old_best_53.model'
    model = load_model(filename)
    psgail = model['model']
    psgail.policy = psgail.policy.cpu()
    psgail.discriminator = psgail.discriminator.cpu()
    psgail.value = psgail.value.cpu()
    psgail.target_value = psgail.target_value.cpu()
    ...
    benchmark(vehicle_id_groups_list, args.agent_num, args.ngsim_path, psgail)
