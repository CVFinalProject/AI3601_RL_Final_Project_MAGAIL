...
class PSGAIL():
    ...
    def update_policy(self, batch, bc_factor, experts):
        state = batch["state"]
        action = batch["action"]
        old_log_prob = batch["log_prob"]
        adv = batch["adv"]
        experts_sample = np.random.randint(0, len(experts), len(state))
        sap_experts = experts[experts_sample, :]
        sap_experts = torch.tensor(sap_experts, device=device, dtype=torch.float32)
        s_experts = sap_experts[:, :-2]
        a_experts = sap_experts[:, -2:]
        _, exp_log_probs, _1, exp_entropy = self.get_action(s_experts, a_experts)
        bc_loss = -exp_log_probs - exp_entropy
        ...
        orig_loss = -torch.mean(torch.min(ip_sp * adv.detach(), ip_sp_clip * adv.detach()))
        policy_loss = ((1 - bc_factor) * orig_loss + bc_factor * bc_loss).mean()
        self.policy_optimizer.zero_grad()
        policy_loss.backward()
        self.policy_optimizer.step()
        return float(policy_loss.data), 0
