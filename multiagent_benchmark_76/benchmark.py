...
from multiagent_benchmark_76.eval_sampler import ParallelPathSampler
from multiagent_benchmark_76.fixed_traffic_simulator import FixedMATrafficSim
from multiagent_benchmark_76.math_utils import DiscreteFrechet
import pickle as pk
import re
import torch
import copy
...
def load_model(model2test):
    with open(model2test, "rb") as f:
        models = torch.load(f, map_location=torch.device('cpu'))
    return models

def batch_test(args, psgail):
...
if __name__ == '__main__':
    ...
    for file_name in file_names:
        if re.search(expr,file_name)!=None:
            psgail = load_model('./best_agent'+file_name)
            print(file_name)
            batch_test(args, psgail)
