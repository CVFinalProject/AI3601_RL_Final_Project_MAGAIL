...
def train_BC_GAIL(psgail, experts, i_episode_res, writer, path, stage='gail', num_episode=200000, print_every=1,
    ...
    for i_episode in range(i_episode_res, num_episode):
        ...
        if (i_episode + 1) % 25 == 0 or i_episode + 1 == num_episode or np.mean(avg_final_log[-2:]) > best_perform or \
                avg_final_log[-1] > best_perform:
            ...
            with open(
                    path + 'psgail_{}_{}_{}_{}_{}.model'.format(i_episode, stage, agent_num,
                                                                int(np.mean(avg_final_log[-2:])), avg_final_log[-1]),
                    "wb") as f:
                pk.dump(
                    {
                        'model': psgail,
