...
def train_BC_GAIL(psgail, experts, i_episode_res, writer, path, stage='gail', num_episode=200000, print_every=1,
    ...
    env_creator = lambda: MATrafficSim_new(["./ngsim"], agent_number=agent_num)
    ...
    for i_episode in range(i_episode_res, num_episode):
        ...
        policy_epoch = max(int(mini_epoch * 200 / i_episode), 8)
        batch_size = max(int(i_episode / 250 * 4096), 16384)
...
if __name__ == "__main__":
    ...
    infos_1 = train_BC_GAIL(psgail, experts, 1054, writer, mods_path, stage='gail')
